...
const float textWidth = 32. / 1024.;
const int texIndex = 10;
static const Vertex QUAD_VERTICES[4] = {
    {{-1, -1}, {textWidth * texIndex, 0}},
    {{1, -1}, {textWidth * (texIndex + 1), 0}},
    {{-1, 1}, {textWidth * texIndex, 1}},
    {{1, 1}, {textWidth * (texIndex + 1), 1}},
};
...
int libMain() {
  Config config;
  config.title = "Basic Rendering";
  config.backend = Engine::Backend::OPENGL;
  App app;
  auto setup = [&app](Engine *engine, View *view, Scene *scene) {
    // Load texture
    // Path path = FilamentApp::getRootAssetsPath() +
    // "textures/Moss_01/Moss_01_Color.png";
    std::cout << "Using root asset path " << FilamentApp::getRootAssetsPath()
              << std::endl;
    ...
    if (data == nullptr) {
      std::cerr << "The texture " << path << " could not be loaded"
                << std::endl;
      return -1;
    }
    ...
    // Set up view
    app.skybox =
        Skybox::Builder().color({0.1, 0.125, 0.25, 1.0}).build(*engine);
    ...
    app.vb = VertexBuffer::Builder()
                 ...
                 .attribute(VertexAttribute::UV0, 0,
                            VertexBuffer::AttributeType::FLOAT2, 8, 16)
                 .build(*engine);
    app.vb->setBufferAt(
        *engine, 0, VertexBuffer::BufferDescriptor(QUAD_VERTICES, 64, nullptr));
